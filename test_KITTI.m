...
for k=1:size(traj,2)-1
    ...
    % match the subclouds
    [corespondencesEdge, edgeWeights] = matchingEdgebis(centeredEdgePoints_1, centeredEdgePoints_2,...
...
%     psiList = [psiList; psi];
    ...
    x0 = [0,0,0,0,0,0];
    lb = [-1.5, -1.5, -0.5, -pi/6, -pi/6, -pi/3];
    ub = [1.5, 1.5, 0.5, pi/6, pi/6, pi/3];
%     edgeWeights = ones(size(edgeWeights));
%     planeWeights = ones(size(planeWeights));
